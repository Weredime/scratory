    ...
    function next(e) {
        ...
        setTimeout(() => {
          if (typeof window !== "undefined") {
            scratchblocks(window).renderMatching("pre.blocks", {
              style: "scratch3"
            })
          }
        }, 0)
              ...
              <div class="grid grid-cols-5">
                <nav class="left-nav p-4 col-span-1">
                  <a href="https://scratch.mit.edu/users/{data.userData.username}">
                    <img src="https://cdn2.scratch.mit.edu/get_image/user/{data.userData.id}_90x90.png" alt={`${data.userData.username}'s profile picture`} />
                    {data.userData.username}
                  </a>
                  <p>{data?.forumData?.counts?.total?.count ?? 1} post{data?.forumData?.counts?.total?.count == null ? "" : "s"}</p>
                </nav>
                <div class="col-span-4 p-4 border-l border-blue-400 w-full h-60">
                  ...
                  <footer class="h-14 text-right">
                    <a href="https://ocular.jeffalo.net/user/{data.userData.username}?utm_campaign=scratory">ocular</a>
                    |
                    <a href="https://magnifier.potatophant.net/users/{data.userData.username}?utm_campaign=scratory">magnifier</a>
                    |
                    <a href="https://postpercent.rirurin.com/users/{data.userData.username}?utm_campaign=scratory">postpercent</a>
                    <br />
                    <input
                      type="range"
                      bind:value={id}
                      max={data.signatureHistory.length - 1}
                      class="w-full"
                      on:change={next}
                    />
                  </footer>

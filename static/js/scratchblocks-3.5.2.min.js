var scratchblocks = (function () {
	...
	*/
	...
	var extensions_1 = {
		...
		movedExtensions: movedExtensions,
	...
	var extensions$1 = extensions_1.extensions,
		aliasExtensions$1 = extensions_1.aliasExtensions;
	...
	var overrideCategories = [
		...
		.concat(Object.keys(extensions$1))
		.concat(Object.keys(aliasExtensions$1));
	...
	disambig('microbit.whenGesture', 'gdxfor.whenGesture', function (children, lang) {
		for (var i = 0; i < children.length; i++) {
			...
			if (child.shape === 'dropdown') {
				var name = child.value; // Yes, "when shaken" gdxfor block exists. But microbit is more common.

				var _iterator3 = _createForOfIteratorHelper(lang.microbitWhen),
					_step3;
	...
	disambig('ev3.buttonPressed', 'microbit.isButtonPressed', function (children, lang) {
		for (var i = 0; i < children.length; i++) {
			var child = children[i];
	...
	specialCase('boost.whenTilted', function () {
		return blocksById['microbit.whenTilted'];
	});
	specialCase('wedo2.whenTilted', function () {
		return blocksById['microbit.whenTilted'];
	});
	specialCase('gdxfor.whenTilted', function () {
		return blocksById['microbit.whenTilted'];
	});
	specialCase('wedo2.isTilted', function () {
		return blocksById['microbit.isTilted'];
	});
	specialCase('gdxfor.isTilted', function () {
		return blocksById['microbit.isTilted'];
	});
	specialCase('boost.getTiltAngle', function () {
		return blocksById['microbit.tiltAngle'];
	});
	specialCase('wedo2.getTiltAngle', function () {
		return blocksById['microbit.tiltAngle'];
	});
	specialCase('gdxfor.getTilt', function () {
		return blocksById['microbit.tiltAngle'];
	}); // It is impossible to see which one is used, so assume it's the common one

	specialCase('makeymakey.whenKeyPressed', function () {
		return blocksById['EVENT_WHENKEYPRESSED'];
	});
	specialCase('boost.getMotorPosition', function () {
		return blocksById['ev3.getMotorPosition'];
	});
	specialCase('ev3.getDistance', function () {
		return blocksById['wedo2.getDistance'];
	});
	specialCase('boost.setLightHue', function () {
		return blocksById['wedo2.setLightHue'];
	});
		...
		if (o) {
			...
			try {
				for (_iterator.s(); !(_step = _iterator.n()).done; ) {
					...
					var outlineChildren = children.splice(
				...
				switch (tok) {
					...
					case '@':
						...
						if (name === 'cloud') {
							...
							children.push(
								Icon$1.icons.hasOwnProperty(name) ? new Icon$1(name) : new Label$1('@' + name)
							);
						}
	...
	var extensions$2 = extensions_1.extensions,
		movedExtensions$1 = extensions_1.movedExtensions,
		aliasExtensions$2 = extensions_1.aliasExtensions;
	var syntax$1 = {
		...
		movedExtensions: movedExtensions$1,
		aliasExtensions: aliasExtensions$2
	};
	...
	var draw_29 = draw.capRect;
	var draw_30 = draw.hatRect;
	var draw_31 = draw.curve;
	var draw_32 = draw.procHatBase;
	var draw_33 = draw.procHatCap;
	var draw_34 = draw.procHatRect;
	var draw_35 = draw.mouthRect;
	var draw_36 = draw.ringRect;
	var draw_37 = draw.commentRect;
	var draw_38 = draw.commentLine;
	var draw_39 = draw.strikethroughLine;
	...
	var Label$3 = syntax$1.Label,
		...
		extensions$3 = syntax$1.extensions,
		movedExtensions$2 = syntax$1.movedExtensions,
		aliasExtensions$3 = syntax$1.aliasExtensions;
	...
	InputView.prototype.draw = function (parent) {
		if (this.hasLabel) {
			...
			var w = Math.max(
				14,
				this.label.width + (this.shape === 'string' || this.shape === 'number-dropdown' ? 6 : 9)
			);
		} else {
	...
	var BlockView = function BlockView(block) {
		...
		if (aliasExtensions$3.hasOwnProperty(this.info.category)) {
			// handle aliases first
			this.info.category = aliasExtensions$3[this.info.category];
		}

		if (movedExtensions$2.hasOwnProperty(this.info.category)) {
			this.info.category = movedExtensions$2[this.info.category];
		} else if (extensions$3.hasOwnProperty(this.info.category)) {
	...
	BlockView.prototype.drawSelf = function (w, h, lines) {
		...
		if (this.isRing) {
			var child = this.children[0]; // We use isStack for InputView; isBlock for BlockView; isScript for ScriptView.
	...
	CommentView.prototype.draw = function () {
		...
		return draw.group([
			draw.commentLine(this.hasBlock ? CommentView.lineLength : 0, 6),
			draw.commentRect(this.width, this.height, {
				class: 'sb-comment'
			}),
	...
	var draw_27$1 = draw$1.capRect;
	var draw_28$1 = draw$1.getHatTop;
	var draw_29$1 = draw$1.getCatTop;
	var draw_30$1 = draw$1.hatRect;
	var draw_31$1 = draw$1.catHat;
	var draw_32$1 = draw$1.getProcHatTop;
	var draw_33$1 = draw$1.procHatRect;
	var draw_34$1 = draw$1.mouthRect;
	var draw_35$1 = draw$1.commentRect;
	var draw_36$1 = draw$1.commentLine;
	var draw_37$1 = draw$1.strikethroughLine;
	...
	var Label$4 = syntax$1.Label,
		...
		extensions$4 = syntax$1.extensions,
		aliasExtensions$4 = syntax$1.aliasExtensions;
	...
	var BlockView$1 = function BlockView(block) {
		...
		if (aliasExtensions$4.hasOwnProperty(this.info.category)) {
			this.info.category = aliasExtensions$4[this.info.category];
		}

		if (extensions$4.hasOwnProperty(this.info.category)) {
	...
	CommentView$1.prototype.draw = function () {
		...
		this.width = this.label.width + 16;
		return draw$1.group([
			draw$1.commentLine(this.hasBlock ? CommentView$1.lineLength : 0, 6),
			draw$1.commentRect(this.width, this.height, {
				class: 'sb3-comment'
			}),
